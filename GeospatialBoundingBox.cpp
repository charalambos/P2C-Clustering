...
void GeospatialBoundingBox::initialize(Image *resampled)	{
	...
	if (!resampled)	{
		...
		Image *filtered_image = ImageProcessing::bilateralFilteringGrayscale(zzz_map,0.25,0.25,0.1,0.1);//1.0,1.0,0.025,0.025);//0.25,0.25,0.025,0.025);
		...
		for (int y=0;y<xyz_map->getHeight();y++)    {
            for (int x=0;x<xyz_map->getWidth();x++) {
                Vector3f pt = color2vector3<float>(xyz_map->getPixel(x,y));
